...
@hydra.main(version_base=None, config_path=CONFIG_PATH,
           config_name="train_datt0615")  # [0626] 切换到6D GT oracle（平动+转动）+ Jacobian约束训练；"train_datt0615_3d"=3D oracle对比基准
def main(cfg):
    ...
    from omni_drones.envs.single.track_datt import TrackDATT  # ensure REGISTRY entry
    algos = {
        "ppo": PPOPolicy,
    ...
    for i, data in enumerate(pbar):
        base_env.count = i # count = the ith updating episode
        ...
        if eval_interval > 0 and i % eval_interval == 0:
            ...
            try:
                ...
                logging.warning("Eval skipped: CUDA out of memory.")
                torch.cuda.empty_cache()
            env.train()
    ...
    try:
        ...
        logging.warning("Final eval skipped: CUDA out of memory.")
        torch.cuda.empty_cache()
    run.log(info)
