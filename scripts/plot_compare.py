...
data = torch.load('/home/jiayu/OmniDrones/scripts/track_sim.pt')
# sim_action_error = torch.load('/home/zanghongzhi/isaac_ws/OmniDrones/scripts/takeoff_sim_action_error.pt')
real_data = torch.load('/home/jiayu/OmniDrones/scripts/takeoff_real.pt')
...
body_rate = data[:-1, 0, 10:13].to('cpu').numpy()
heading = data[:-1, 0, 13:16].to('cpu').numpy()
lateral = data[:-1, 0, 16:19].to('cpu').numpy()
up = data[:-1, 0, 19:22].to('cpu').numpy()
...
# sim body rate
axs[19].plot(time_steps, body_rate[:, 0], label='sim roll rate')
# axs[16].plot(real_time_steps, real_up[:, 0], label='real up x')
axs[19].legend()

axs[20].plot(time_steps, body_rate[:, 1], label='sim pitch rate')
# axs[17].plot(real_time_steps, real_up[:, 1], label='real up y')
axs[20].legend()

axs[21].plot(time_steps, body_rate[:, 2], label='sim yaw rate')
# axs[18].plot(real_time_steps, real_up[:, 2], label='real up z')
axs[21].legend()
