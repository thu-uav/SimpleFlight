...
# [修改] 类名改为 TrackPINN
class TrackPINNTest0518(IsaacEnv):
    ...
    def switch_eval_traj(self, traj_name: str):
        """动态切换评估轨迹类型，无需重启 Isaac Sim。
        调用后需执行 env.reset() 使所有 env 生效新轨迹。
        仅在 use_eval=True 模式下有效。
        支持: 'normal', 'fast', 'pentagram'
        """
        assert self.use_eval, "switch_eval_traj 仅在 use_eval=True 模式下有效"
        self.eval_traj = traj_name
        self.traj_t0 = torch.zeros(self.num_envs, 1, device=self.device)
        if traj_name == 'normal':
            self.ref = Lemniscate(T=5.5, origin=self.origin, device=self.device)
            self.traj_t0 = torch.ones(self.num_envs, 1, device=self.device) * 5.5 / 4
            ...
            raise ValueError(f"未知轨迹类型: {traj_name}，支持 'normal'/'fast'/'pentagram'")
        print(f"[switch_eval_traj] 已切换到轨迹类型: {traj_name}")
