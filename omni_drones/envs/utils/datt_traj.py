...
class RandomZigzag(BaseRef):
    ...
    def reset(self, env_ids):
        ...
        pos_low_x = torch.rand((num_trajs, self.size // 2, 1)) * (-self.max_D[0]) - self.max_D[0]
        pos_high_y = torch.rand((num_trajs, self.size // 2, 1)) * self.max_D[1]
        pos_low_y = torch.rand((num_trajs, self.size // 2, 1)) * (-self.max_D[1]) - self.max_D[1]
        pos_high_z = torch.rand((num_trajs, self.size // 2, 1)) * self.max_D[2]
        pos_low_z = torch.rand((num_trajs, self.size // 2, 1)) * (-self.max_D[2]) - self.max_D[2]
...
class ChainedPolyRef(BaseRef):
    def __init__(self, altitude, use_y=False, min_dt=1.5, max_dt=4.0, degree=3, seed=2023, env_diff_seed=False, fixed_seed=False, **kwargs):
        offset_pos = kwargs.get('offset_pos', np.zeros(3))
        super().__init__(offset_pos)
        assert degree % 2 == 1
...
if __name__ == '__main__':
    ...
    # ref = ChainedPolyRef(altitude=0, use_y=True, seed=0) # smooth trajectories
    t = torch.linspace(0, 2, 1000) # 0~10s, 最后一维足够多即可，可以拟合整个线段
