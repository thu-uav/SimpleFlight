...
max_thrust = 1.0
# sim data
sim_target = torch.load('/home/jiayu/OmniDrones/real2sim/sim_action.pt')
sim_real = torch.load('/home/jiayu/OmniDrones/real2sim/sim_rpy.pt')
...
if not plot_sim:     
    start_T_real = 0
    end_T_real = 0 + 1800
    df = pd.read_csv('/home/jiayu/OmniDrones/real2sim/real_fast.xls', skip_blank_lines=True)
    ...
    preprocess_df = df.to_numpy()[start_T_real:end_T_real]
    real_target = preprocess_df[:, 46:49]
    real_thrust = preprocess_df[:, 49] / 15.0
    # real_thrust = preprocess_df[:, 42]
    # real_target = preprocess_df[:, 43:46]
    real_real_rpy = preprocess_df[:, 50:]
    real_time_steps = np.arange(len(real_real_rpy))
...
if plot_sim:
    ...
    axs[0].plot(time_steps, sim_target_rpy[:len(time_steps), 0], label='sim target roll rate')
    axs[0].legend()

    # axs[1].plot(time_steps, sim_real_rpy[:, 1], label='sim pitch rate')
    axs[1].plot(time_steps, sim_target_rpy[:len(time_steps), 1], label='sim target pitch rate')
    axs[1].legend()

    # axs[2].plot(time_steps, sim_real_rpy[:, 2], label='sim yaw rate')
    axs[2].plot(time_steps, sim_target_rpy[:len(time_steps), 2], label='sim target yaw rate')
    axs[2].legend()
    
    axs[3].plot(time_steps, sim_target_thrust[:len(time_steps)], label='sim target thrust')
